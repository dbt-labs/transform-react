...
export type UsePercentChangeQueryState = {
  ...
  data?: FetchPercentChangeQuery["mqlQuery"] | null;
...
export const initialState: UsePercentChangeQueryState = {
  retries: 0,
  queryId: null,
...
export type Action =
  ...
  | { type: "postQueryCachedResultsSuccess";
      data: CreatePercentChangeMutation;
...
*/
const percentChangeReducer = () => (
  state: UsePercentChangeQueryState,
  action: Action,
): UsePercentChangeQueryState => {
  switch (action.type) {
    case "postQueryStart": {
    ...
    case "postQueryCachedResultsSuccess": {
      return {
        ...
        data: action.data?.pctChangeOverRange?.query,
        fetchStartTime: null,
        isTakingForever: false,
        errorMessage: undefined,
      }
...
export default percentChangeReducer;
